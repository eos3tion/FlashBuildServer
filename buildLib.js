...
function make7z(sourceDir, zipFile) {
    ...
    args[4] = zipFile;
    args[5] = sourceDir + "\\*";
    var obj = childProcess.spawnSync("7z", args, {
        stdio: 'inherit',
        cwd: OTHER_EXEC
    });
    if (obj.error) {
...
function ssh(host, username, password, commands, stdout) {
    ...
    args[3] = password;
    args[4] = commands;
    var obj = childProcess.spawnSync("plink", args, {
...
function svnUpdate(distDir, stdout) {
    ...
    args[4] = "buider";
    args[5] = distDir;
    return childProcess.spawnSync("svn", args, {
        stdio: ['inherit', stdout, 'inherit']
    });
}
...
function mxmlc(source, output, isDebug, sources, defines, metaDatas, linkLibs, linkReports, loadExterns, swfVersion) {
    ...
    var obj = childProcess.spawnSync("mxmlc", args, {
        stdio: 'inherit',
        cwd: FLEX_BIN_PATH
    });
    if (obj.error) {
...
function lzmaSWF(source, dist) {
    ...
    args[1] = "-f";
    args[2] = dist;
    var obj = childProcess.spawnSync("zswf.exe", args, {
        stdio: 'inherit',
        cwd: OTHER_EXEC
    });
    if (obj.error) {
...
function obfuscation(oswf, dswf, dir, isMain) {
    ...
    var obj = childProcess.spawnSync("as3cJY.exe", args, {
        stdio: 'inherit',
        cwd: OTHER_EXEC
    });
    if (obj.error) {
...
function processDir(source, target, tempDir, ext) {
    ...
    args[3] = tempDir;
    args[4] = ext;
    var obj = childProcess.spawnSync("python", args, {
        stdio: 'inherit',
        cwd: PYTHON_2
    });
    if (obj.error) {
